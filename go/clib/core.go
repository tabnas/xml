...
//
// libtabnasxml is the xml format parser as a C shared library, one of
...
// newParser builds one engine with the xml grammar installed
...
func newParser() (parseFn, error) {
	j := host.Make(); if err := j.UseDefaults(plug.Xml, plug.Defaults); err != nil { return nil, err }; return j.Parse, nil
}
...
// parseWith answers whether src is in the xml language — and,
