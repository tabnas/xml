...
- **Nesting stops at 256 open elements.** A deeper document fails with
  the engine's `cancel` code. The engine walks a nested value by
  recursion to display, convert, clone, compare or drop it, and drops its
  rule snapshots the same way, and a stack overflow ends the process
  where nothing can catch it. Displaying a value about 400 elements deep
  overflowed a 2 MiB thread's stack in a debug build. 256 is the depth
  libxml2 allows by default. TypeScript and Go accept any depth. No
  document a person writes comes near the limit, and a self-closed
  element, which opens and closes at once, doesn't count toward it.
